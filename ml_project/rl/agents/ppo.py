...
from rl.networks.network import Actor, Critic
from rl.utils.utils import ReplayBuffer, convert_to_tensor, make_mini_batch
...
class PPO(nn.Module):
    def __init__(self, writer, device, state_dim, action_dim, args):
        ...
        self.actor_optimizer = optim.Adam(
            self.actor.parameters(), lr=self.args.actor_lr
        )
    ...
    def get_gae(self, states, rewards, next_states, dones):
        ...
        advantages = torch.Tensor(advantage_lst).to(self.device)
        return values, advantages

    def train_net(self, n_epi):
        data = self.data.sample(shuffle=False)
        states, actions, rewards, next_states, dones, old_log_probs = convert_to_tensor(
            self.device,
            data["state"],
            data["action"],
            data["reward"],
        ...
        for i in range(self.args.train_epoch):
            for (
                ...
                surr2 = (
                    torch.clamp(ratio, 1 - self.args.max_clip, 1 + self.args.max_clip)
                    * advantage
                )
                actor_loss = (-torch.min(surr1, surr2) - entropy).mean()

                # value clipping (PPO2 technic)
                old_value_clipped = old_value + (value - old_value).clamp(
                    -self.args.max_clip, self.args.max_clip
                )
                value_loss = (value - return_.detach().float()).pow(2)
                value_loss_clipped = (old_value_clipped - return_.detach().float()).pow(
                    2
                )
                critic_loss = (
                    0.5
                    * self.args.critic_coef
                ...
                if self.writer is not None:
                    ...
                    self.writer.add_scalar(
                        "loss/critic_loss", critic_loss.item(), n_epi
                    )
