...
class SAC(nn.Module):
    def __init__(self, writer, device, state_dim, action_dim, args):
        ...
        self.soft_update(self.q_1, self.target_q_1, 1.0)
        self.soft_update(self.q_2, self.target_q_2, 1.0)
    ...
    def soft_update(self, network, target_network, rate):
        for network_params, target_network_params in zip(
            ...
            target_network_params.data.copy_(
                target_network_params.data * (1.0 - rate) + network_params.data * rate
            )
    ...
    def alpha_update(self, now_action_log_prob):
        loss = (
            -self.alpha * (now_action_log_prob + self.target_entropy).detach()
        ).mean()
    ...
    def train_net(self, batch_size, n_epi):
        ...
        ###q update
        q_1_loss = self.q_update(
            self.q_1, self.q_1_optimizer, states, actions, rewards, next_states, dones
        )
        q_2_loss = self.q_update(
            self.q_2, self.q_2_optimizer, states, actions, rewards, next_states, dones
        )
