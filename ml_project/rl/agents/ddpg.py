...
from rl.utils.utils import ReplayBuffer, convert_to_tensor
...
class DDPG(nn.Module):
    def __init__(self, writer, device, state_dim, action_dim, args, noise):
        ...
        self.soft_update(self.q, self.target_q, 1.0)
    ...
    def get_action(self, x):
        return (
            self.actor(x)[0] + torch.tensor(self.noise.sample()).to(self.device),
            self.actor(x)[1],
        )
    ...
    def train_net(self, batch_size, n_epi):
        ...
        targets = rewards + self.args.gamma * (1 - dones) * self.target_q(
            next_states, self.target_actor(next_states)[0]
        )
